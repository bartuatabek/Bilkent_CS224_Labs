##	_main - a program that calls linked list utility functions,
...
MenuZ:	
	...
	la $a0,msg121	# put msg117 address into a0
	li $v0,4	# system call to print
	syscall		# out the msg117 string
	
	la $a0,msg122	# put msg117 address into a0
	li $v0,4	# system call to print
	syscall		# out the msg117 string
...
T7:	bne $s1,7, T8
	...
	move $a0, $s0	# put pointer to linked list in a0 before the call
	jal display_reverse_list
	
	la $a0,msg85	# put msg85 address into a0
	li $v0,4	# system call to print
	syscall		# out the string
...
T8:	bne $s1,8, T9
	...
	move $a0, $s0	# put pointer to linked list in a0 before the call
	jal display_reverse_list_recursive
	
	lw $a0, 4($t0)	# read the value part, put into a0
	li $v0, 1	# system call to print  
	syscall		# out the integer
	
	la $a0, msg123	# put msg85 address into a0
	li $v0, 4	# system call to print
	syscall		# out the msg85 string
...
T9:	bne $s1,9, T9no	# if s1 = 7, do these things. Else go to T7no
...
Insert_end:
	addi $sp,$sp,-8 # make room on stack for 3 new items	
	sw   $s0, 8 ($sp)  # push $s0 value onto stack
	sw   $s1, 4 ($sp)  # push $s1 value onto stack
		
	move $s0, $a0	# the pointer to the first element in the list is put in $s0
	
	bne $s0, $zero, alloc
	
	lw   $s0, 8 ($sp) # restore $s0 value from stack
	lw   $s1, 4 ($sp) # restore $s1 value from stack
	addi $sp, $sp, 8  # restore $sp to original value (i.e. pop 3 items)
	li   $v0, -1
	jr   $ra 	  # return to point of call
...
alloc:	
	...
	move $t0, $v0	# the final value of list pointer is put in $s0
	lw   $s1, ($s0)		
	
	sw $a1, 4($t0) 	 # store the value from user into current element's value part
	sw $zero, 0($t0) # Put Null value into pointerToNext part of last element in list
			
loop:	
	beq $s1, $zero, insert # if end of list return
...
insert:	
	sw $t0, 0($s0)	   # the previous element's pointerToNext is loaded with the new element's address
...
delHead:
	li  $t0, 1	       # load value in head to $t0
	bne $t0, $a1, traverse # if head not equal input traverse the list
	
	move  $s0, $s1    # swap head with next
	move  $s1, $s0	
	addi  $sp, $sp, 8 # restore $sp to original value (i.e. pop 2 items)
...
traverse:
	...
	addi $t0, $t0, 1      # load the next value to $t0
...
delete:
	...
	sw   $s2, 0($s0) # the element's pointerToNext is loaded with the next element's address
...
return:
	...
	move $v1, $s0 # return the head of list in $v1
	li   $v0, -1   # return -1 to indicate failure
	jr   $ra
...
Delete_x:
	...
	move $s0, $a0		 # put the pointer to the current element in $s0
	lw   $s1, ($s0)		 # read the value of pointerToNext
	bne  $s0, $zero, delHead2 # if pointer is NULL, there is no list

	j return
	
delHead2:
	lw  $t0, 4($s0)	       # load value in head to $t0
	bne $t0, $a1, traverse2 # if head no equal input traverse the list
...
# display_reverse_list subprogram
#------------------------------------------------
display_reverse_list:	
	li $t0, 0

	move $t1, $a0	   # put the pointer to the current element in $s0
	lw   $t2, ($t1)	   # read the value of pointerToNext
	lw   $t3, 4($t1)
push:	
	addi $sp, $sp, -4      # make room on stack for 3 new items
	sw   $t3, 0($sp)       # push $ra value onto stack
	
	beq  $t2, $zero, pop # if end of list return
	
	addi $t0, $t0, 1
	move $t1, $t2 # continue traversing
	lw   $t2, ($t1)
	lw   $t3, 4($t1)
...
pop:
	blt  $t0, $zero, return3 # if end of list return
	lw   $t3, 0($sp)         # push $ra value onto stack
	addi $sp, $sp, 4         # make room on stack for 3 new items
	
	move $a0, $t3	# read the value part, put into a0
	li   $v0, 1	# system call to print  
	syscall		# out the integer
	
	la $a0, msg123	# put msg123 address into a0
	li $v0, 4	# system call to print
	syscall		# out the msg123 string
...
# display_reverse_list_recursive subprogram
#------------------------------------------------
display_reverse_list_recursive:
	addi $sp, $sp, -12 # make room on stack for 3 new items
	sw   $ra, 8($sp)   # push $ra value onto stack
	
	move $t0, $a0	   # put the pointer to the current element in $s0
	lw   $t1, ($t0)	   # read the value of pointerToNext
	
	sw   $t1, 4($sp)  # push $ra value onto stack
	sw   $t0, 0($sp)  # push $ra value onto stack
	
	bne  $t1, $zero, else # if input is greater than 1 go to else
	
	addi $sp, $sp, 12 # restore $sp to original value (i.e. pop 3 items)
	jr   $ra
	
else:	
	move $t0, $t1 # continue traversing
	lw   $t1, ($t0)
	move $a0, $t0

	jal  display_reverse_list_recursive # recursive call to subprogram
	...
	lw   $ra, 8($sp)  # push $ra value onto stack
	lw   $t1, 4($sp)  # push $ra value onto stack
	lw   $t0, 0($sp)  # push $ra value onto stack
	addi $sp, $sp, 12 # restore $sp to original value (i.e. pop 2 items)
	jr   $ra
...
msg121:	 .asciiz "7 - display reverse order iteratively \n"
msg122:	 .asciiz "8 - display reverse order recursively \n"
